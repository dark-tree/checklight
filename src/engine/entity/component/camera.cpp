...
void Camera::onUpdate(Context c) {
	float distance_multiplier = c.delta * speed * speed_modifier;
...
InputResult Camera::onEvent(const InputEvent& event) {
	if(const auto* key_event = event.as<KeyboardEvent>()) {
		...
		if(key_event->wasPressed(GLFW_KEY_W)) pressed_forward = true;
			...
			//down
		else if(key_event->wasPressed(GLFW_KEY_Q) || key_event->wasPressed(GLFW_KEY_LEFT_SHIFT))
			//TODO make two separate ifs?
			pressed_down = true;
