...
float planeDistanceWeight(vec3 centerPos, vec3 samplePos, vec3 normal) {
	return 1 - min(abs(dot(centerPos - samplePos, normal)) * 4, 1);
}

vec4 edgeAvoidingBlur(int radius, float stepSize, vec2 center, float centerDepth, vec3 centerNormal,
	sampler2D normalsSampler, sampler2D illuminationSampler, sampler2D positionSampler, vec3 centerWS) {
	...
	for (int x = -radius; x <= radius; x++) {
		for (int y = -radius; y <= radius; y++) {
			...
			vec2 pos = center + vec2(x, y) * illumTexelSize * stepSize;
